...
int DmaBufPool::Allocate(size_t frame_size) {
...

  {
    ...
    if (fn_alloc && fn_get_fd && fn_mmap) {
      ...
      const int bpp = 12;
      // Derive w/h from frame_size so allocation matches the requested size
      // instead of always allocating 640x480. Total pixels = frame_size * 8 / bpp.
      const int w = static_cast<int>(frame_size * 8 / bpp);
      const int h = 1;
      bool ok = true;
