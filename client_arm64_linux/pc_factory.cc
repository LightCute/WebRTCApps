...
// From rk_mpp_stubs.cc — runtime MPP loader
bool MppLibLoad();
...
PcComponents PcFactory::Create(webrtc::Thread* network_thread,
  ...
  bool has_mpp = MppLibLoad();

  if (has_mpp) {
    RTC_LOG(LS_INFO) << "MPP hardware codec detected, using MppH264";
    ...
    RTC_LOG(LS_INFO) << "MPP not available, using software codecs";
    deps.video_encoder_factory =
        std::make_unique<webrtc::VideoEncoderFactoryTemplate<
            webrtc::LibvpxVp8EncoderTemplateAdapter,
            webrtc::LibvpxVp9EncoderTemplateAdapter,
