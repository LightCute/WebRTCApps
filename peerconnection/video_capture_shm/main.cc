// video_capture_shm — standalone video capture → SHM, no PeerConnection deps
#include <atomic>
#include <csignal>
#include <cstdlib>
#include <iostream>
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "video_capture_shm starting" << std::endl;
  ...
  // 2. Create SHM sink (ShmVideoRenderer wraps ShmVideoWriter + IO thread)
  ShmVideoRenderer sink(shm_key, shm_proj_id);

  // 3. Register sink with capturer
  webrtc::VideoSinkWants wants;
  capturer->AddOrUpdateSink(&sink, wants);
