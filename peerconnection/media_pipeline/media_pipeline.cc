...
#include "apps/peerconnection/media_pipeline/shm_common.h"
...
bool MediaPipeline::CreateAudioDeviceModule(int audio_layer) {
  auto layer = static_cast<webrtc::AudioDeviceModule::AudioLayer>(audio_layer);
  if (audio_layer == 0) {
    layer = webrtc::AudioDeviceModule::kPlatformDefaultAudio;
  }
  adm_ = worker_thread_->BlockingCall([this, layer] {
    return webrtc::CreateAudioDeviceModule(env_, layer);
  });
  if (!adm_) {
...
void MediaPipeline::Shutdown() {
  ...
  if (adm_ && worker_thread_) {
    ...
    worker_thread_->BlockingCall([adm = std::move(adm)]() mutable {
      // Always call Stop regardless of Playing()/Recording().
      // StartPlayout/StartRecording set internal flags before calling
      // platform code. If platform fails, Playing() returns false but
      // the internal AudioDeviceBuffer::playing_ flag is still true,
      // causing a DCHECK in the destructor.
      adm->StopPlayout();
      adm->StopRecording();
      adm = nullptr;
    });
