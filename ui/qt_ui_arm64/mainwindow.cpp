...
void MainWindow::initUi() {
    ...
    ui.local_video_->setStyleSheet(
        "border: 2px solid rgba(255,255,255,0.25); border-radius: 8px;");
    ui.local_video_->hide();
    ui.stats_label_->setStyleSheet(
    ...
    connect(channel_, &ControlChannel::dataReceived, this,
        ...
        if (text == "AI:ON:yolov5") {
            ...
            if (err.error == QJsonParseError::NoError) {
                ...
                ui.local_video_->setDetections(dets);
            }
        }
    });
...
void MainWindow::startVideoSources() {
    ...
    connect(local_source_, &DmaBufVideoSource::frameReady, this,
        ...
        local_frame_count_++;
        if (ui.local_video_) {
            ui.local_video_->setFrame(f);
            if (!ui.local_video_->isVisible()) {
                ui.local_video_->show();
                // Position at top-right; delay slightly to ensure layout is done
                QTimer::singleShot(100, this, [this]() {
                    int x = ui.callPage->width() - ui.local_video_->width() - 20;
                    int y = 10;
                    ui.local_video_->move(qMax(0, x), y);
                    ui.local_video_->raise();
                });
            }
            ui.local_video_->raise();
        }
...
void MainWindow::onAiDetections(QVector<Detection> detections) {
    ui.local_video_->setDetections(detections);
