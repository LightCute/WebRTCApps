...
机器人端以 Rockchip RK3588 为核心计算单元，自主设计 STM32 MCU 控制板和电机驱动板，搭载四轮差速底盘和二维舵机云台。软件方面，采用 Qt5 框架构建人机交互界面，以进程管理和跨进程通信（Unix Socket、DMA-BUF 共享内存、UART 串口）协同多个功能模块：WebRTC 音视频通话引擎、三组 AI 视觉推理模型（行人检测 YOLOv5、跌倒检测 YOLOv11、火灾烟雾检测 YOLOv8）、语音对话系统（本地 Whisper 语音识别 + 云端大语言模型 + 本地语音合成）。视频管线采用 DMA-BUF 零拷贝架构，通过 RGA 硬件模块完成色彩空间转换，MPP 硬件编码器实现 H.264 编码，全程 CPU 零参与像素数据搬运。

系统实现了四大核心功能：（1）公网实时音视频通话与远程底盘/云台控制，监护者可通过键盘远程操控机器人移动和摄像头视角；（2）AI 代理行人追踪模式，开启后机器人自动跟随行人移动，将目标保持在画面中央，提升通话体验；（3）跌倒检测和火灾烟雾检测两种安全监护模式，异常事件触发后自动推送告警至监护端；（4）独立语音对话模式，无人连接时机器人可进行自然语言陪聊。

本系统面向居家养老场景，通过异构芯片协同（RK3588 负责音视频和 AI 推理，MCU 负责底盘实时控制）、零拷贝视频管线优化性能和功耗、多 AI 模型在线热切换覆盖核心安全需求，为独居老人提供远程陪伴和智能守护。
...
**（1）异构芯片协同架构。** RK3588 负责高计算量的音视频处理、AI 推理和 UI 渲染，自研 STM32 MCU 负责底盘实时控制和传感器采集。两者通过 UART 115200 串口通信，定义自定义帧协议（0xAA 0x55 帧头 + XOR 校验 + 300ms 看门狗），职责边界清晰。

**（2）DMA-BUF 零拷贝视频管线。** 摄像头采集的 YUYV 帧通过 RGA 硬件模块转换为 NV12，写入 CMA 分配的 DMA-BUF 内存。DMA-BUF 文件描述符（fd）随 `Nv12DmaBufBuffer` 穿越整个 WebRTC 管线——从 RGA 转换到 MPP 硬件编码器，数据始终留在 GPU/硬件可访问的物理连续内存中，全程 CPU 不参与像素搬运。

**（3）多 AI 模型在线热切换。** 三种 YOLO 系列模型（行人/跌倒/火灾）在 RK3588 NPU 上运行，通过 QProcess 管理推理进程生命周期。用户点击 UI 按钮即可切换模型，无需重启系统或重新加载。

**（4）事件驱动的结构化远程控制。** 定义 5 种控制命令（move/ptz/move_ptz/ptz_home/stop），PC 端按下按键即发送 JSON 指令，松开即发送停止指令。支持组合键（W+J=前进+云台左转），80ms 保活周期配合 MCU 300ms 看门狗确保可靠性。

## 1.4 主要性能指标

| 指标 | 数值 | 说明 |
|------|------|------|
| 视频分辨率 | 640×480 @30fps | V4L2 原生输出 |
| 编码格式 | H.264 | MPP 硬件编码，DMA-BUF 零拷贝 |
| AI 推理间隔 | 250ms | RK3588 NPU, YOLO Anchor-Free 解码 |
| AI 模型数量 | 3 种 | 可在线热切换 |
| 控制响应 | 事件驱动 | 按下即发，松开即停止 |
| MCU 通信 | UART 115200 | 0xAA 0x55 自定义帧协议 |
| 语音对话 | Whisper 本地 + LLM 云端 | 支持多轮记忆 |
| 公网连接 | WebRTC P2P + STUN/TURN | HTTP 长轮询信令 |
